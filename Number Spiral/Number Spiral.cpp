...
#define f1(i,s,e) for(long long int i=s;i<e;i++)
#define f2(i,s,e) for(long long int i=s;i<e-1;i++)
#define cf(i,s,e) for(long long int i=s;i<=e;i++)
#define rf(i,e,s) for(long long int i=e-1;i>=s;i--)
#define fb(j,s,e) for(long long int j=s+1;i<e;i++)
    ...
    while(ttt--)
{
    ...
    {
        ll answer;
        if(col%2!=0)answer=(col*col)-row+1;
        else answer=((col-1)*(col-1))+row;cout<<answer<<"\n";
    }
    else
    {
        ll answer;
        if(row%2!=0)answer=((row-1)*(row-1))+col;
        else answer=(row*row)-col+1;cout<<answer<<"\n";
    }
}
    return 0;
}